...
public class NetworkCommand {
                                        ...
                                        .then(Commands.argument("target", GameProfileArgument.gameProfile())
                                                .suggests(
                                                        (commandContext, suggestionsBuilder) -> {
                                                            ...
                                                            UserNameToIdResolver profileCache = commandContext.getSource()
                                                                    .getServer().services().nameToIdCache();
                                                            return SharedSuggestionProvider.suggest(
                                                                    members.stream()
                                                                            .map(member -> profileCache.get(member.playerId())
                                                                                    .map(NameAndId::name)
                                                                                    .orElse(member.playerId().toString())),
                                                                    suggestionsBuilder
                                                            );
                                                        }
                                                        ...
                                                        (commandContext, suggestionsBuilder) -> {
                                                            ...
                                                            return SharedSuggestionProvider.suggest(
                                                                    playerList.getPlayers()
                                                                            .stream()
                                                                            .map(serverPlayer -> serverPlayer.nameAndId().name()),
                                                                    suggestionsBuilder
                                                            );
                                                        }
                                                )
    ...
    private static int listNetworkMembers(CommandContext<CommandSourceStack> context) throws CommandSyntaxException {
        ...
        List<String> list = theatricalNetwork.members().members()
                ...
                .map(playerId -> profileCache.get(playerId).map(NameAndId::name).orElse(playerId.toString()))
                .toList();
    ...
    private static boolean isSourceOperator(CommandContext<CommandSourceStack> context) {
        return context.getSource().permissions().hasPermission(new Permission.HasCommandLevel(
                context.getSource().getServer().operatorUserPermissions().level()));
    }
